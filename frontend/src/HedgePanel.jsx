...
function getUrgency(pctVariance) {
  ...
  if (pctVariance < 18) return { level: "moderate", label: "Moderate", color: "#fbbf24" };
  if (pctVariance < 30) return { level: "significant", label: "Significant", color: "#f97316" };
  return { level: "dominant", label: "Dominant", color: "#ff4757" };
}
...
const FACTOR_COLORS = ["#6366f1", "#fbbf24", "#00e68a", "#ff4757", "#8b5cf6"];
    ...
    <div className="hedge-panel hedge-panel-summary">
      <div className="hedge-panel-header">
        ...
        <div className="hedge-panel-header-actions">
          {activeFactor && (
            <button className="hedge-summary-btn" onClick={() => setActiveFactor(null)}>
              ← Summary
            </button>
          )}
          <button className="hedge-close-btn" onClick={onClose}>&times;</button>
